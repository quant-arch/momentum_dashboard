...
# Daily portfolio file path - FOR STREAMLIT CLOUD DEPLOYMENT
# Looks for portfolio_data.xlsx in the same directory as this script
DAILY_PORTFOLIO_FILE = os.path.join(BASE_DIR, "portfolio_data.xlsx")
...
# --- TrueData Client (Embedded to avoid import issues) ---
import requests
import time
import logging
import struct
...
# Try to import lz4 (optional for decompression)
try:
    import lz4.block
    HAS_LZ4 = True
except ImportError:
...
class TrueDataClient:
    ...
    def _calculate_start_time(self, duration, end_dt):
        ...
        if unit == 'M':
            return end_dt - timedelta(days=val*30)
        if unit == 'Y':
            return end_dt - timedelta(days=val*365)
            
        return end_dt - timedelta(days=365)

# --- Get TrueData Credentials ---
...
def get_current_portfolio_from_file(uploaded_file=None):
    """
    Read current portfolio stocks from Excel file (in memory only).
    Priority: 1) Uploaded file 2) Google Drive 3) Local file
    Args:
        uploaded_file: Optional Streamlit UploadedFile object
    Returns: tuple (list of stock tickers, latest date, dataframe, source)
    """
    try:
        # Priority 1: User uploaded file (manual override)
        file_to_read = None
        source = None
        daily_df = None
        ...
        if uploaded_file is not None:
            ...
            source = "uploaded"
            daily_df = pd.read_excel(file_to_read)
        
        # Priority 2: Google Drive (automated cloud) - REMOVED
        # elif USE_GOOGLE_DRIVE and GDRIVE_FOLDER_ID:
        #     pass 

        
        # Priority 3: Local file (automated local)
        if daily_df is None and os.path.exists(DAILY_PORTFOLIO_FILE):
            file_to_read = DAILY_PORTFOLIO_FILE
            source = "local"
            daily_df = pd.read_excel(file_to_read)
        ...
        logging.error(f"Error reading daily portfolio file: {e}")
        return None, None, None, None
...
def main():
    ...
    if cache_exists and cached_end_date:
        st.sidebar.success(f"✅ Cache valid\nLast updated: {cached_updated[:19]}")
    else:
    ...
    # Optional: Upload custom portfolio file
    st.sidebar.subheader("📂 Custom Portfolio (Optional)")
    
    # Auto-check for daily_momentum_report.xlsx
    daily_report_path = os.path.join(BASE_DIR, "daily_momentum_report.xlsx")
    auto_loaded = False
    if os.path.exists(daily_report_path):
        st.sidebar.success("✅ Using daily_momentum_report.xlsx")
        auto_loaded = True
    
    uploaded_daily_file = st.sidebar.file_uploader(
        "Or upload custom portfolio" if auto_loaded else "Upload portfolio Excel",
        type=['xlsx'],
        help="Auto-loads 'daily_momentum_report.xlsx' if available, or upload your own"
    )
    
    # Use auto-loaded file if no upload
    if uploaded_daily_file is None and auto_loaded:
        uploaded_daily_file = daily_report_path
    
    if uploaded_daily_file:
        st.sidebar.success("✅ Using uploaded portfolio")
    ...
    with tab3:
        ...
        # For Streamlit Cloud: uses portfolio_data.xlsx in same directory
        excel_path = DAILY_PORTFOLIO_FILE
        portfolio_entries = {}
