...
for target_file in files_to_update:
    ...
    # Fix cell 0 import pyodbc
    if len(nb['cells']) > 0 and nb['cells'][0]['cell_type'] == 'code':
        nb['cells'][0]['source'] = [line for line in nb['cells'][0]['source'] if 'import pyodbc' not in line]
    ...
    # Inject cell 5 (execution)
    if len(nb['cells']) > 5 and nb['cells'][5]['cell_type'] == 'code':
        nb['cells'][5]['source'] = [line + '\\n' for line in cell_5_code.split('\\n')]
        if len(nb['cells'][5]['source']) > 0:
            nb['cells'][5]['source'][-1] = nb['cells'][5]['source'][-1].rstrip('\\n')
            
    # Inject cell 10 (hedge)
    if len(nb['cells']) > 10 and nb['cells'][10]['cell_type'] == 'code':
    ...
    # Clear cell 15 (redundant buy_value calculation)
    if len(nb['cells']) > 15 and nb['cells'][15]['cell_type'] == 'code':
        nb['cells'][15]['source'] = []
