...
# Read the notebook
with open('Momentum_UPDATED.ipynb', encoding='utf-8') as f:
    nb = json.load(f)

# Cell 2 content (process_portfolio + build_weighted_hedge_segment + build_rebalanced_hedge_book)
...
# Cell 10 content (calling build_rebalanced_hedge_book)
cell_10_code = """# Build hedge book using the rebalancing function
...
if len(nb['cells'][2]['source']) > 0:
    nb['cells'][2]['source'][-1] = nb['cells'][2]['source'][-1].rstrip('\\n')

# Wait, `prepare_and_process_portfolio` is in Cell 3.
# Need to check `prepare_and_process_portfolio` because `process_portfolio` parameters might be slightly different.
# In `scratch_script2.py`, `prepare_and_process_portfolio` calls `process_portfolio` like this:
...
# The new `process_portfolio` from Script 1 accepts `(nav_df, ticker_data, initial_value=75, inception_date=None, output_file=None)`. 
# It expects `inception_date`. I'll update Cell 3 just to pass `inception_date`.
