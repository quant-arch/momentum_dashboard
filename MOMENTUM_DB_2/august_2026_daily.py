"""
august_2026_daily.py
====================
Copied from: july_2026_daily.py

August 2026 Rebalance Logic
---------------------------
On 2026-08-01 (monthly rebalance):

HEDGE PORTFOLIO:
  - DEFENCE is sold and the entire sale proceeds are used to buy ITBEES.
    (DEFENCE's July 31, 2026 Buy_Hold_Value transfers 1:1 into ITBEES.)
  - Other hedge instruments (LIQUIDCASE, NEXT50IETF, PHARMABEES) continue
    carrying forward their July 31, 2026 values into August.

  NOTE: 2026-08-01 is a Saturday. The August hedge segment is anchored on the
  first actual trading day (2026-08-03) so ITBEES's day-1 return is measured
  Open -> Close as a fresh buy, not as a close-to-close gap from July 31.

EQUITY PORTFOLIO:
  - 10 stocks exit the portfolio:
      ABB, ABSLAMC, BANDHANBNK, BSE, FINCABLES,
      GVT&D, J&KBANK, JINDALSAW, KIRLOSENG, MAHABANK
  - 10 new stocks enter, splitting the pooled July 31 Buy_Hold_Value of the
    10 exits equally:
      ACMESOLAR, ADANIENSOL, ADANIGREEN, CUMMINSIND, GRANULES,
      KAJARIACER, LAURUSLABS, LLOYDSME, PREMIERENE, SONACOMS

  Four of the exiting stocks (BANDHANBNK, FINCABLES, J&KBANK, JINDALSAW)
  entered in July and had their values rescaled by the July correction below,
  so process_portfolio's own July-end values for them are stale. The August
  correction re-pools the *corrected* July 31 values before splitting.

--- Inherited from july_2026_daily.py -------------------------------------

July 2026 Rebalance Logic
--------------------------
On 2026-07-01 (monthly rebalance):

EQUITY PORTFOLIO:
  - All 5 Vedanta entities (VEDL, VAML, VISL, VEDPOWER, VOGL) are sold as
    ONE combined slot — they all originated from a single VEDL position,
    so they count as 1 freed slot (not 5).
  - 8 other stocks also exited the portfolio:
  ...
  - Combined Buy_Hold_Value of all 9 slots on June 30, 2026 is pooled and
    divided by 9  →  new Buy_Hold per incoming stock = 10.521
  - 9 new stocks enter the portfolio, each allocated 10.521:
      ATHERENERG, BANDHANBNK, CPPLUS, FINCABLES, J&KBANK,
      JINDALSAW, RRKABEL, SYRMA, WELCORP

  NOTE: ABSLAMC has been kept in place of HFCL in the July 2026 portfolio.

HEDGE PORTFOLIO:
  - GOLDBEES Buy_Hold_Value is fully transferred to DEFENCE on 2026-07-01.
  - Other hedge instruments (LIQUIDCASE, NEXT50IETF, PHARMABEES) continue
    carrying forward their June 30, 2026 values into July.
...
# ============================================================
# 2.  JULY 2026 REBALANCE CONSTANTS
# ============================================================
...
# → new Buy_Hold per stock = combined_june30_BHV / 9 = 10.521
JULY_NEW_SLOTS = 9

# ============================================================
# 2b. AUGUST 2026 REBALANCE CONSTANTS
# ============================================================
# 2026-08-01 falls on a Saturday; the first August trading day is 2026-08-03.
# Equity rows are still keyed off Aug 1 (that is the selection date in
# master_momentum_summary), while the hedge segment resolves its own start
# date from the price data.
AUGUST_REBALANCE_DATE = pd.Timestamp('2026-08-01')

# 10 stocks leaving the portfolio after July 2026
AUGUST_DROPPED = [
    'ABB', 'ABSLAMC', 'BANDHANBNK', 'BSE', 'FINCABLES',
    'GVT&D', 'J&KBANK', 'JINDALSAW', 'KIRLOSENG', 'MAHABANK',
]

# 10 new stocks entering on August 1, 2026
AUGUST_NEW_STOCKS = [
    'ACMESOLAR', 'ADANIENSOL', 'ADANIGREEN', 'CUMMINSIND', 'GRANULES',
    'KAJARIACER', 'LAURUSLABS', 'LLOYDSME', 'PREMIERENE', 'SONACOMS',
]
...
# Hedge: DEFENCE is sold in full and the proceeds buy ITBEES.
AUGUST_HEDGE_SELL = 'DEFENCE'
AUGUST_HEDGE_BUY  = 'ITBEES'

# ============================================================
# 3.  TRUEDATA FETCH HELPER
# ============================================================
...
def build_rebalanced_hedge_book(base_portfolio_df, portfolio_end_date=None, default_hedge_value=25.0):
    ...
    # DEFENCE added for the July 2026 GOLDBEES → DEFENCE hedge transfer
    # ITBEES added for the August 2026 DEFENCE → ITBEES hedge transfer
    hedge_prices = fetch_truedata_history(
        ticker_list=[
            'GOLDBEES', 'SILVERBEES', 'MOGSEC', 'LIQUIDCASE',
            'CPSEETF', 'PHARMABEES', 'NEXT50IETF', 'DEFENCE', 'ITBEES',
        ],
        duration='5 Y',
        bar_size='EOD',
        sleep_time=0.1
    ...
    # June segment is capped at June 30 so the July transfer can start cleanly on July 1.
    june_start = pd.Timestamp('2026-06-01')
    june_end   = min(pd.Timestamp('2026-06-30'), portfolio_end_date)
    ...
    if portfolio_end_date >= june_start:
        ...
        if cpse_value > 0.0:
            ...
            df_june = build_weighted_hedge_segment(
                hedge_prices, start_date=june_start, end_date=june_end,
                base_values=june_values, segment_name='2026-06',
    ...
    # ------------------------------------------------------------------
    # July 2026: Transfer GOLDBEES Buy_Hold_Value → DEFENCE
    # Other hedge instruments (LIQUIDCASE, NEXT50IETF, PHARMABEES) carry
    # forward their June 30 Buy_Hold_Values into July unchanged.
    # July segment is capped at July 31 so the August transfer starts cleanly.
    # ------------------------------------------------------------------
    july_start = pd.Timestamp('2026-07-01')
    july_end   = min(pd.Timestamp('2026-07-31'), portfolio_end_date)
    df_july    = pd.DataFrame()
    if portfolio_end_date >= july_start and not df_june.empty:
        june_last_date = df_june['Date'].max()
        june_last      = (
            df_june[df_june['Date'] == june_last_date]
            .set_index('Ticker')['Buy_Hold_Value']
            .to_dict()
        )

        goldbees_bhv = june_last.get('GOLDBEES', 0.0)

        # GOLDBEES BHV transferred in full to DEFENCE; GOLDBEES exits the hedge.
        july_values = {k: v for k, v in june_last.items() if k != 'GOLDBEES'}
        july_values['DEFENCE'] = july_values.get('DEFENCE', 0.0) + goldbees_bhv

        logging.info(f"[July Hedge] GOLDBEES BHV on {june_last_date.date()}: {goldbees_bhv:.4f} -> DEFENCE")
        logging.info(f"[July Hedge] July hedge allocations: {july_values}")

        df_july = build_weighted_hedge_segment(
            hedge_prices,
            start_date=july_start,
            end_date=july_end,
            base_values=july_values,
            segment_name='2026-07',
            new_tickers=['DEFENCE'],   # DEFENCE bought fresh at Open on July 1
        )

        if not df_july.empty:
            # Fix Buy_Price and Quantity for DEFENCE (bought at Open on July 1)
            def_prices = hedge_prices[
                (hedge_prices['Ticker'] == 'DEFENCE') & (hedge_prices['Date'] >= july_start)
            ].sort_values('Date')
            def_open = float(def_prices.iloc[0]['Open']) if not def_prices.empty else None

            def_mask = df_july['Ticker'] == 'DEFENCE'
            if def_mask.any() and def_open:
                df_july.loc[def_mask, 'Buy_Price'] = def_open
                df_july.loc[def_mask, 'Quantity']  = np.where(
                    def_open > 0,
                    df_july.loc[def_mask, 'Initial_Allocation'] / def_open,
                    0.0
                )

            segments.append(df_july)

    # ------------------------------------------------------------------
    # August 2026: Sell DEFENCE, buy ITBEES with the full sale proceeds.
    # Other hedge instruments (LIQUIDCASE, NEXT50IETF, PHARMABEES) carry
    # forward their July 31 Buy_Hold_Values into August unchanged.
    # ------------------------------------------------------------------
    df_aug = pd.DataFrame()
    if portfolio_end_date >= AUGUST_REBALANCE_DATE and not df_july.empty:
        july_last_date = df_july['Date'].max()
        july_last      = (
            df_july[df_july['Date'] == july_last_date]
            .set_index('Ticker')['Buy_Hold_Value']
            .to_dict()
        )

        defence_bhv = july_last.get(AUGUST_HEDGE_SELL, 0.0)

        # DEFENCE BHV transferred in full to ITBEES; DEFENCE exits the hedge.
        aug_values = {k: v for k, v in july_last.items() if k != AUGUST_HEDGE_SELL}
        aug_values[AUGUST_HEDGE_BUY] = aug_values.get(AUGUST_HEDGE_BUY, 0.0) + defence_bhv

        # 2026-08-01 is a Saturday. Anchor the segment on the first actual
        # trading day so ITBEES's day-1 return is Open -> Close (a fresh buy)
        # rather than a close-to-close gap carried over from July 31.
        aug_dates = hedge_prices.loc[hedge_prices['Date'] >= AUGUST_REBALANCE_DATE, 'Date']
        if aug_dates.empty:
            logging.warning(f"[Aug Hedge] No hedge prices on/after "
                            f"{AUGUST_REBALANCE_DATE.date()} — skipping August segment.")
        else:
            aug_start = aug_dates.min()

            logging.info(f"[Aug Hedge] {AUGUST_HEDGE_SELL} BHV on {july_last_date.date()}: "
                         f"{defence_bhv:.4f} -> {AUGUST_HEDGE_BUY}")
            logging.info(f"[Aug Hedge] Segment start (first trading day): {aug_start.date()}")
            logging.info(f"[Aug Hedge] August hedge allocations: {aug_values}")

            df_aug = build_weighted_hedge_segment(
                hedge_prices,
                start_date=aug_start,
                end_date=portfolio_end_date,
                base_values=aug_values,
                segment_name='2026-08_onward',
                new_tickers=[AUGUST_HEDGE_BUY],   # ITBEES bought fresh at Open
            )

            if not df_aug.empty:
                # Fix Buy_Price and Quantity for ITBEES (bought at Open on day 1)
                it_prices = hedge_prices[
                    (hedge_prices['Ticker'] == AUGUST_HEDGE_BUY) & (hedge_prices['Date'] >= aug_start)
                ].sort_values('Date')
                it_open = float(it_prices.iloc[0]['Open']) if not it_prices.empty else None

                it_mask = df_aug['Ticker'] == AUGUST_HEDGE_BUY
                if it_mask.any() and it_open:
                    df_aug.loc[it_mask, 'Buy_Price'] = it_open
                    df_aug.loc[it_mask, 'Quantity']  = np.where(
                        it_open > 0,
                        df_aug.loc[it_mask, 'Initial_Allocation'] / it_open,
                        0.0
                    )

                segments.append(df_aug)
...
# ============================================================
# 6.  JULY 2026 EQUITY REBALANCE CORRECTION
# ============================================================
def apply_july_2026_rebalance_correction(conc_df_final: pd.DataFrame) -> pd.DataFrame:
    """
    Corrects July 2026 new-stock allocations in conc_df_final.
    ...
    This function:
      ...
      2. Reads the June 30 Buy_Hold_Values for all 9 freed slots:
           ...
           - 8 individual dropped stocks: ADANIENSOL, ADANIGREEN, CUMMINSIND,
             HINDALCO, MCX, NATIONALUM, NAVINFLUOR, SAIL
      3. Computes correct_new_alloc = total_freed_BHV / 9  (target: 10.521)
      4. Scales Buy_Hold_Value, Initial_Allocation and Quantity for the 9
         new July stocks to match correct_new_alloc.
      5. Recomputes Total_Portfolio_Value for all dates after the correction.
    """
    conc_df = conc_df_final.copy()
    conc_df['Date'] = pd.to_datetime(conc_df['Date'])

    # ------------------------------------------------------------------
    # Step 1: Drop Vedanta entity rows from July 1 onwards
    #         (All 5 are sold at the July 2026 rebalance)
    # ------------------------------------------------------------------
    vedanta_july_mask = (
        conc_df['Ticker'].isin(VEDANTA_GROUP_TICKERS) &
        (conc_df['Date'] >= JULY_REBALANCE_DATE)
    )
    n_removed = vedanta_july_mask.sum()
    conc_df = conc_df[~vedanta_july_mask].copy()
    ...
    # ------------------------------------------------------------------
    june_rows = conc_df[conc_df['Date'].dt.to_period('M').astype(str) == '2026-06']
    if june_rows.empty:
    ...
    june_snap = (
        conc_df[conc_df['Date'] == june_last_date]
        .set_index('Ticker')['Buy_Hold_Value']
    )
    ...
    print(f"\n{'='*65}")
    print("  JULY 2026 EQUITY REBALANCE SUMMARY")
    print(f"{'='*65}")
    ...
    print(f"  New Buy_Hold per stock (/9)       : {correct_new_alloc:.4f}  [target: 10.521]")
    ...
    #         (it only included VEDL's BHV, not the full Vedanta group)
    # ------------------------------------------------------------------
    july_new_mask = (
        conc_df['Ticker'].isin(JULY_NEW_STOCKS) &
        (conc_df['Date'] >= JULY_REBALANCE_DATE)
    )
    ...
    process_alloc = float(conc_df.loc[july_new_mask, 'Initial_Allocation'].iloc[0])
    if process_alloc <= 0:
    ...
    # ------------------------------------------------------------------
    conc_df.loc[july_new_mask, 'Buy_Hold_Value']    *= scale_factor
    conc_df.loc[july_new_mask, 'Quantity']           *= scale_factor
    conc_df.loc[july_new_mask, 'Initial_Allocation'] = correct_new_alloc

    logging.info(f"[July Rebalance] Scaled {july_new_mask.sum()} rows for "
                 f"{conc_df.loc[july_new_mask, 'Ticker'].nunique()} new July stocks. "
                 f"scale_factor={scale_factor:.6f}")

    # ------------------------------------------------------------------
    # Step 6: Recompute Total_Portfolio_Value for ALL dates
    # ------------------------------------------------------------------
    daily_total = conc_df.groupby('Date')['Buy_Hold_Value'].sum().rename('_tpv')
    conc_df = conc_df.merge(daily_total, on='Date', how='left')
    conc_df['Total_Portfolio_Value'] = conc_df['_tpv']
    conc_df.drop(columns=['_tpv'], inplace=True)

    return conc_df


# ============================================================
# 6b. AUGUST 2026 EQUITY REBALANCE CORRECTION
# ============================================================
def apply_august_2026_rebalance_correction(conc_df_final: pd.DataFrame) -> pd.DataFrame:
    """
    Corrects August 2026 new-stock allocations in conc_df_final.

    Must run AFTER apply_july_2026_rebalance_correction().

    process_portfolio() sizes the 10 incoming August stocks from its own
    July-end Buy_Hold_Values for the 10 outgoing stocks. Four of those exits
    (BANDHANBNK, FINCABLES, J&KBANK, JINDALSAW) entered in July and were
    rescaled by the July correction, so process_portfolio's values for them
    are stale and the pooled amount is wrong.

    This function re-pools the *corrected* July 31 Buy_Hold_Values of all 10
    exits, divides by 10, and rescales the incoming stocks to match.
    """
    conc_df = conc_df_final.copy()
    conc_df['Date'] = pd.to_datetime(conc_df['Date'])

    # ------------------------------------------------------------------
    # Step 1: Get July 31 (last trading day of July) snapshot
    # ------------------------------------------------------------------
    july_rows = conc_df[conc_df['Date'].dt.to_period('M').astype(str) == '2026-07']
    if july_rows.empty:
        logging.warning("[Aug Rebalance] No July 2026 rows found — skipping correction.")
        return conc_df

    july_last_date = july_rows['Date'].max()
    july_snap = (
        conc_df[conc_df['Date'] == july_last_date]
        .set_index('Ticker')['Buy_Hold_Value']
    )

    # ------------------------------------------------------------------
    # Step 2: Compute the correct new allocation
    # ------------------------------------------------------------------
    total_freed_bhv   = sum(float(july_snap.get(t, 0.0)) for t in AUGUST_DROPPED)
    correct_new_alloc = total_freed_bhv / AUGUST_NEW_SLOTS

    print(f"\n{'='*65}")
    print("  AUGUST 2026 EQUITY REBALANCE SUMMARY")
    print(f"{'='*65}")
    print(f"  Last July trading day             : {july_last_date.date()}")
    print(f"  10 dropped stocks BHV (corrected July 31):")
    for t in AUGUST_DROPPED:
        print(f"    {t:<12}: {float(july_snap.get(t, 0.0)):.4f}")
    print(f"  Total freed BHV (10 slots)        : {total_freed_bhv:.4f}")
    print(f"  New Buy_Hold per stock (/10)      : {correct_new_alloc:.4f}")
    print(f"  10 new stocks entering August     : {AUGUST_NEW_STOCKS}")

    # ------------------------------------------------------------------
    # Step 3: Find the allocation process_portfolio used for August new stocks
    #         (it pooled stale, pre-July-correction values)
    # ------------------------------------------------------------------
    aug_new_mask = (
        conc_df['Ticker'].isin(AUGUST_NEW_STOCKS) &
        (conc_df['Date'] >= AUGUST_REBALANCE_DATE)
    )

    if not aug_new_mask.any():
        logging.warning("[Aug Rebalance] No August new stock rows found — "
                        "skipping Buy_Hold correction.")
        return conc_df

    aug_first_date = conc_df.loc[aug_new_mask, 'Date'].min()
    process_alloc  = float(
        conc_df.loc[aug_new_mask & (conc_df['Date'] == aug_first_date),
                    'Initial_Allocation'].iloc[0]
    )
    if process_alloc <= 0:
        logging.warning(f"[Aug Rebalance] process_alloc={process_alloc} invalid — skipping.")
        return conc_df

    scale_factor = correct_new_alloc / process_alloc

    print(f"\n  First August trading day                  : {aug_first_date.date()}")
    print(f"  process_portfolio alloc (stale basis)     : {process_alloc:.4f}")
    print(f"  Correction scale factor                   : {scale_factor:.6f}")
    print(f"{'='*65}\n")

    # ------------------------------------------------------------------
    # Step 4: Apply scale to all August new-stock rows
    # ------------------------------------------------------------------
    conc_df.loc[aug_new_mask, 'Buy_Hold_Value']    *= scale_factor
    conc_df.loc[aug_new_mask, 'Quantity']           *= scale_factor
    conc_df.loc[aug_new_mask, 'Initial_Allocation'] = correct_new_alloc

    logging.info(f"[Aug Rebalance] Scaled {aug_new_mask.sum()} rows for "
                 f"{conc_df.loc[aug_new_mask, 'Ticker'].nunique()} new August stocks. "
                 f"scale_factor={scale_factor:.6f}")

    # ------------------------------------------------------------------
    # Step 5: Recompute Total_Portfolio_Value for ALL dates
    # ------------------------------------------------------------------
    daily_total = conc_df.groupby('Date')['Buy_Hold_Value'].sum().rename('_tpv')
    conc_df = conc_df.merge(daily_total, on='Date', how='left')
    conc_df['Total_Portfolio_Value'] = conc_df['_tpv']
...
def main():
    ...
    # August 2026 output file (same name the daily workflow / Power BI reads)
    ...
    print("  July 2026:   Vedanta group exits as 1 slot; GOLDBEES -> DEFENCE hedge")
    print("  August 2026: 10 equity slots turn over; DEFENCE -> ITBEES hedge")
    print("="*65)
    ...
    print("  (June capped at Jun 30; July: GOLDBEES BHV -> DEFENCE, capped at Jul 31)")
    print("  (August: DEFENCE BHV -> ITBEES, from the first Aug trading day)")
    print("="*65)
    ...
    # ----------------------------------------------------------------
    # STEP 9: Apply July 2026 equity rebalance correction
    #         - Sells all Vedanta entities (VEDL, VAML, VISL, VEDPOWER, VOGL)
    #           as ONE combined slot (originated from single VEDL position)
    #         - Pools combined BHV of 9 freed slots on June 30 -> /9 -> 10.521
    #         - Scales the 9 new July stocks to the correct allocation
    #         - GOLDBEES -> DEFENCE transfer is already handled in hedge book
    # ----------------------------------------------------------------
    conc_df_final = apply_july_2026_rebalance_correction(conc_df_final)

    print(f"conc_df_final shape (after July rebalance correction): {conc_df_final.shape}")

    # ----------------------------------------------------------------
    # STEP 9b: Apply August 2026 equity rebalance correction
    #         - Re-pools the CORRECTED July 31 BHV of the 10 exiting stocks
    #           (4 of them were rescaled by the July correction above)
    #         - Splits that pool /10 across the 10 incoming August stocks
    #         - DEFENCE -> ITBEES transfer is already handled in hedge book
    # ----------------------------------------------------------------
    conc_df_final = apply_august_2026_rebalance_correction(conc_df_final)

    print(f"conc_df_final shape (after August rebalance correction): {conc_df_final.shape}")
    print(f"Unique tickers post-correction: {sorted(conc_df_final['Ticker'].unique())}")

    # ----------------------------------------------------------------
    # STEP 10: Save July 2026 output
    # ----------------------------------------------------------------
    print("\n" + "="*65)
    ...
    # ---------------------------
    print("\n" + "="*65)
    print("  STEP 11: Portfolio summary")
    print("="*65)
    ...
    for _, row in latest_rows.iterrows():
        flag = " << ITBEES (from DEFENCE)" if row['Ticker'] == AUGUST_HEDGE_BUY else ""
        flag = flag or (" << NEW AUGUST" if row['Ticker'] in AUGUST_NEW_STOCKS else "")
        flag = flag or (" << NEW JULY" if row['Ticker'] in JULY_NEW_STOCKS else "")
    ...
    # July new stock allocation check on July 1
    july1_data = conc_df_final[
        (pd.to_datetime(conc_df_final['Date']) == pd.Timestamp('2026-07-01')) &
        (conc_df_final['Ticker'].isin(JULY_NEW_STOCKS))
    ][['Ticker', 'Initial_Allocation', 'Buy_Hold_Value']].copy()
    if not july1_data.empty:
        print(f"\nJuly 1 allocation check for new stocks (target: 10.521 each):")
        print(july1_data.to_string(index=False))

    # August new stock allocation check on the first August trading day
    aug_rows = conc_df_final[
        (pd.to_datetime(conc_df_final['Date']) >= AUGUST_REBALANCE_DATE) &
        (conc_df_final['Ticker'].isin(AUGUST_NEW_STOCKS))
    ]
    if not aug_rows.empty:
        aug_first = pd.to_datetime(aug_rows['Date']).min()
        aug1_data = aug_rows[pd.to_datetime(aug_rows['Date']) == aug_first][
            ['Ticker', 'Initial_Allocation', 'Buy_Hold_Value']
        ].copy()
        print(f"\nAugust allocation check for new stocks on {aug_first.date()}:")
        print(aug1_data.to_string(index=False))

    # August hedge check: DEFENCE sold, ITBEES bought with the proceeds
    hedge_check = conc_df_final[
        conc_df_final['Ticker'].isin([AUGUST_HEDGE_SELL, AUGUST_HEDGE_BUY])
    ].copy()
    if not hedge_check.empty:
        hedge_check['Date'] = pd.to_datetime(hedge_check['Date'])
        print(f"\nAugust hedge transfer check ({AUGUST_HEDGE_SELL} -> {AUGUST_HEDGE_BUY}):")
        for tkr in [AUGUST_HEDGE_SELL, AUGUST_HEDGE_BUY]:
            leg = hedge_check[hedge_check['Ticker'] == tkr].sort_values('Date')
            if leg.empty:
                continue
            print(f"  {tkr:<10} {leg['Date'].min().date()} -> {leg['Date'].max().date()}  "
                  f"first BHV={leg.iloc[0]['Buy_Hold_Value']:.4f}  "
                  f"last BHV={leg.iloc[-1]['Buy_Hold_Value']:.4f}")

    # ----------------------------------------------------------------
    # STEP 12: Benchmark — NIFTY 500
    # ----------------------------------------------------------------
    print("\n" + "="*65)
    print("  STEP 12: Fetching NIFTY 500 benchmark")
    print("="*65)
    ...
    # ----------------------------------------------------------------
    # STEP 13: Summary comparison table
    #   Columns: Date | Total_Portfolio_Value | Benchmark_Change
    #   Sources: conc_df_final (equity+hedge) and nse (NIFTY 500)
    # ----------------------------------------------------------------
    print("\n" + "="*65)
    print("  STEP 13: Building Portfolio vs Benchmark summary table")
    print("="*65)

    # Total Portfolio Value per day = sum of all tickers' Buy_Hold_Value
    port_daily = (
        conc_df_final
    ...
    # Benchmark Change from nse (already computed above)
    bench_daily = nse[['Date', 'Benchmark Change']].copy()
    bench_daily['Date'] = pd.to_datetime(bench_daily['Date'])

    # Merge on Date (inner join keeps only trading days common to both)
    ...
    summary_out = os.path.join(OUTPUT_FOLDER, "Portfolio_vs_Benchmark_August2026.xlsx")
    summary.to_excel(summary_out, index=False)
