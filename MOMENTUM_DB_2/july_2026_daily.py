...
july_2026_daily.py
==================
Copied from: Momentum_daily_June_rebalance_vedl_demerger.py
...
def main():
    # ----------------------------------------------------------------
    # GUARD: this script only models the book through July 2026.
    # It shares its output filename with the later monthly scripts, so
    # running it after July silently overwrites the current month's
    # numbers with stale July logic (e.g. DEFENCE never being sold).
    # ----------------------------------------------------------------
    if date.today() > date(2026, 7, 31):
        print("\n" + "!"*65)
        print("  july_2026_daily.py is OUT OF DATE for today's rebalance.")
        print(f"  Today is {date.today()}; this script stops modelling at 2026-07-31.")
        print("  Running it would overwrite the shared output file")
        print("  'Nifty_500_2025_Apr_20_stocks_results_GoldSilverDebt_buy&hold_returns.xlsx'")
        print("  with July logic (DEFENCE is never sold, ITBEES is never bought).")
        print("\n  Run the CURRENT month's script instead, e.g.:")
        print("      python august_2026_daily.py")
        print("!"*65 + "\n")
        raise SystemExit(1)
    ...
    print("  (June capped at June 30; July: GOLDBEES BHV → DEFENCE)")
    print("="*65)
    ...
    for _, row in latest_rows.iterrows():
        flag = " << DEFENCE (from GOLDBEES)" if row['Ticker'] == 'DEFENCE' else ""
    ...
    summary_out = os.path.join(OUTPUT_FOLDER, "Portfolio_vs_Benchmark_July2026.xlsx")
