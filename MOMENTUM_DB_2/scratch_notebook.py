...
def build_rebalanced_hedge_book(base_portfolio_df, portfolio_end_date=None, default_hedge_value=25.0):
    ...
    hedge_prices = fetch_truedata_history(
        ticker_list=['GOLDBEES', 'SILVERBEES', 'MOGSEC', 'LIQUIDCASE', 'CPSEETF', 'NEXT50IETF'],
        duration='5 Y',
        bar_size='EOD',
        sleep_time=0.1
    )[0]
    ...
    if portfolio_end_date >= may_start and not df_mar.empty:
        ...
        # May rebalancing: keep hedge total unchanged, move from
        # 10% GOLDBEES + 15% LIQUIDCASE to 5% GOLDBEES + 10% LIQUIDCASE
        # and deploy the proceeds into 5% CPSEETF + 5% NEXT50IETF of total portfolio.
        # Because the hedge is 25% of portfolio, these portfolio weights correspond
        # to 20/40/20/20 of the hedge allocation.
        may_values = {
