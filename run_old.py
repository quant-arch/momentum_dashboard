    ...
    for ticker in ticker_list:
        try:
            ...
            if not {'Date', 'Close', 'Open'}.issubset(df.columns):
                logging.warning(
                    f'Missing required price columns for {ticker}: {list(df.columns)}')
                error_list.append(ticker)
                continue
...
if portfolio_end_date >= mar_start and not df_feb.empty:
    ...
    df_mar = df_mar[(df_mar['Date'] >= mar_start) & (df_mar['Date'] <= min(pd.Timestamp('2026-04-30'), portfolio_end_date))].copy()
...
# --- May 2026 hedge rebalance: move into CPSEETF and NEXT50IETF ---
...
if portfolio_end_date >= may_start and not df_mar.empty:
    ...
    raw_df_may = fetch_truedata_history(
        ticker_list=['GOLDBEES', 'LIQUIDCASE', 'CPSEETF', 'NEXT50IETF'],
        duration='5 Y',
        bar_size='EOD',
        sleep_time=0.1
    )[0]

    if raw_df_may is None or raw_df_may.empty:
        df_may = pd.DataFrame()
    else:
        rename_dict = {}
        if 'timestamp' in raw_df_may.columns:
            rename_dict['timestamp'] = 'Date'
        elif 'datetime' in raw_df_may.columns:
            rename_dict['datetime'] = 'Date'
        elif 'date' in raw_df_may.columns:
            rename_dict['date'] = 'Date'
        if 'high' in raw_df_may.columns:
            rename_dict['high'] = 'High'
        if 'low' in raw_df_may.columns:
            rename_dict['low'] = 'Low'
        if 'close' in raw_df_may.columns:
            rename_dict['close'] = 'Close'
        if 'open' in raw_df_may.columns:
            rename_dict['open'] = 'Open'

        df_may = raw_df_may.rename(columns=rename_dict)
        required_cols = {'Date', 'Ticker', 'Open', 'Close'}
        if not required_cols.issubset(df_may.columns):
            print('Warning: May fetch missing required columns:', list(df_may.columns))
            df_may = pd.DataFrame()
        else:
            df_may = df_may[['Date', 'Ticker', 'Open', 'Close']].copy()
            df_may = df_may[(df_may['Date'] >= may_start) & (df_may['Date'] <= portfolio_end_date)].copy()
            df_may['%change'] = df_may.groupby('Ticker')['Close'].pct_change()
            ...
            df_may['BaseValue'] = df_may['Ticker'].map(may_values)
            df_may['%change'] = pd.to_numeric(df_may['%change'])
            df_may = df_may.sort_values(['Ticker', 'Date'])
            df_may['ret_factor'] = 1 + df_may['%change'].fillna(0)
            df_may['cum_factor'] = df_may.groupby('Ticker')['ret_factor'].cumprod()
            df_may['Value_On_Date'] = df_may['BaseValue'] * df_may['cum_factor']
            df_may = df_may[['Date', 'Ticker', 'Open', 'Close', 'Value_On_Date', '%change']].rename(columns={'Value_On_Date': 'Buy_Hold_Value'})

    df = pd.concat([df_decjan, df_feb, df_mar, df_may], ignore_index=True).sort_values(['Date', 'Ticker'])

# TEMPORARY WORKAROUND: TrueData API is failing for May tickers
# Forward-fill April hedge holdings through May to maintain portfolio continuity
# This keeps the hedge allocation stable until TrueData recovers

may_start = pd.Timestamp('2026-05-01')
if portfolio_end_date >= may_start and df_may.empty and not df_mar.empty:
    print("WARNING: May hedge fetch failed (TrueData API issue)")
    print("Applying workaround: forward-filling April hedge through May...")
    
    # Get the last day of March/April
    mar_last_date = df_mar['Date'].max()
    mar_last = df_mar[df_mar['Date'] == mar_last_date].copy()
    
    # Create May dates (from May 1 to portfolio end)
    may_dates = pd.date_range(
        start=may_start,
        end=portfolio_end_date,
        freq='B'  # Business days only
    )
    
    # Replicate April 30 holdings for each May business day (no price changes)
    df_may_forward = []
    for may_date in may_dates:
        day_copy = mar_last.copy()
        day_copy['Date'] = may_date
        day_copy['%change'] = 0.0  # No change for forward-filled data
        df_may_forward.append(day_copy)
    
    df_may = pd.concat(df_may_forward, ignore_index=True) if df_may_forward else pd.DataFrame()
    print(f"Created {len(df_may)} forward-filled May rows ({len(may_dates)} business days)")
    
    # UPDATE: Re-concatenate df to include the updated df_may
    df = pd.concat([df_decjan, df_feb, df_mar, df_may], ignore_index=True).sort_values(['Date', 'Ticker'])
    print(f"Updated df to include May hedge: {df.shape[0]} total rows")


if 'df_may' in globals() and not df_may.empty:
    print('--- May hedge segment summary ---')
    print('Tickers:', sorted(df_may['Ticker'].unique()))
    print('Date range:', df_may['Date'].min(), 'to', df_may['Date'].max())
    first_date = df_may['Date'].min()
    first_values = df_may[df_may['Date'] == first_date].set_index('Ticker')['Buy_Hold_Value']
    total_first_value = first_values.sum()
    print('Total hedge value on first May row:', total_first_value)
    print('May allocation values:')
    print(first_values)
    if total_first_value > 0:
        print('May allocation percentages:')
        print((first_values / total_first_value * 100).round(2))
else:
    print('May hedge segment is empty or not defined yet.')
...
print("=" * 70)
print("HEDGE REBALANCING ISSUE DIAGNOSIS")
print("=" * 70)

# Check each segment
print(f"\ndf_decjan (Dec 2025 - Jan 2026):")
print(f"  Shape: {df_decjan.shape}")
print(f"  Tickers: {df_decjan['Ticker'].unique()}")
print(f"  Date range: {df_decjan['Date'].min()} to {df_decjan['Date'].max()}")

print(f"\ndf_feb (Feb 2026):")
print(f"  Shape: {df_feb.shape}")
print(f"  Tickers: {df_feb['Ticker'].unique()}")
print(f"  Date range: {df_feb['Date'].min()} to {df_feb['Date'].max()}")

print(f"\ndf_mar (Mar - Apr 2026):")
print(f"  Shape: {df_mar.shape}")
print(f"  Tickers: {df_mar['Ticker'].unique()}")
print(f"  Date range: {df_mar['Date'].min()} to {df_mar['Date'].max()}")

print(f"\ndf_may (May 2026):")
print(f"  Shape: {df_may.shape}")
if not df_may.empty:
    print(f"  Tickers: {df_may['Ticker'].unique()}")
    print(f"  Date range: {df_may['Date'].min()} to {df_may['Date'].max()}")
else:
    print(f"  ** EMPTY - TrueData fetch returned no data **")

# Analyze the drop
print("\n" + "=" * 70)
print("PORTFOLIO VALUE AT KEY TRANSITIONS")
print("=" * 70)

dates_to_check = [
    pd.Timestamp('2026-04-29'),  # Before last Apr date
    pd.Timestamp('2026-04-30'),  # Last Apr date (last day of Mar segment)
    pd.Timestamp('2026-05-01'),  # First May date
    pd.Timestamp('2026-05-02'),  # Second May date
]

for check_date in dates_to_check:
    if check_date in conc_df['Date'].values:
        daily = conc_df[conc_df['Date'] == check_date]
        total = daily['Buy_Hold_Value'].sum()
        print(f"\n{check_date.date()}:")
        print(f"  Tickers present: {daily['Ticker'].unique()}")
        print(f"  Total value: {total:.2f}")
        for _, row in daily.iterrows():
            print(f"    {row['Ticker']:12s}: {row['Buy_Hold_Value']:8.2f}")
    else:
        print(f"\n{check_date.date()}: NO DATA")

print("\n" + "=" * 70)
print("ROOT CAUSE SUMMARY")
print("=" * 70)
if df_may.empty:
    print("ISSUE: df_may is empty")
    print("REASON: TrueData API failed to fetch May hedge ticker data")
    print("  - GOLDBEES, LIQUIDCASE, CPSEETF, NEXT50IETF all returned empty")
    print("IMPACT: May hedge segment missing from portfolio")
    print("  - Portfolio drops from ~104 to ~74 on May 1-2")
    print("  - Only equity portion (old_df) remains, hedge is not tracked")
else:
    print("May hedge data IS present")
    print("Analyzing data quality and calculations...")

# Quick check: is df_may empty?
print("df_may shape:", df_may.shape)
print("df_may empty?", df_may.empty)
print()

# Check conc_df May data
may_data = conc_df[conc_df['Date'] >= '2026-05-01']
print("May data in conc_df:")
print(f"  Rows: {len(may_data)}")
print(f"  Unique tickers: {may_data['Ticker'].unique()}")
print(f"  Unique dates: {may_data['Date'].nunique()}")
print()

# Check if May has hedge tickers
hedge_tickers = ['GOLDBEES', 'LIQUIDCASE', 'CPSEETF', 'NEXT50IETF', 'SILVERBEES', 'MOGSEC']
may_hedge = may_data[may_data['Ticker'].isin(hedge_tickers)]
print(f"May hedge tickers in conc_df: {len(may_hedge)} rows")
if len(may_hedge) > 0:
    print(f"  Tickers found: {may_hedge['Ticker'].unique()}")
else:
    print("  ** NO HEDGE TICKERS IN MAY **")
...
conc_df.to_excel('OLD.xlsx', index=False)